...
public sealed class TokenProvider : IDisposable, IMediatorSubscriber
{
    ...
    public async Task<string> GetNewToken(bool isRenewal, JwtIdentifier identifier, CancellationToken ct)
    {
        ...
        try
        {
            if (!isRenewal)
            {
                ...
                _logger.LogDebug("Server Auth at: {auth}", _serverManager.CurrentAuthServiceUri);

                if (!_serverManager.CurrentServer.UseOAuth2)
                {
                    tokenUri = MareAuth.AuthFullPath(new Uri(_serverManager.CurrentAuthServiceUri));
                ...
                {
                    tokenUri = MareAuth.AuthWithOauthFullPath(new Uri(_serverManager.CurrentAuthServiceUri));
        ...
        catch (HttpRequestException ex)
        {
            ...
            {
                if (isRenewal)
                    Mediator.Publish(new NotificationMessage("Error refreshing token", "Your authentication token could not be renewed. Try reconnecting to PlayerSync manually.",
                    NotificationType.Error));
                else
                    Mediator.Publish(new NotificationMessage("Error generating token", $"Your authentication token could not be generated. Check PlayerSync Main UI ({CommandAlias.Active} in chat) to see the error message.",
                    NotificationType.Error));
        ...
        if (tokenTime <= dateTimeMinus10 || tokenTime >= dateTimePlus10)
        {
            ...
            Mediator.Publish(new NotificationMessage("Invalid system clock", "The clock of your computer is invalid. " +
                "PlayerSync will not function properly if the time zone is not set correctly. " +
    ...
    public async Task<bool> TryUpdateOAuth2LoginTokenAsync(ServerStorage currentServer, bool forced = false)
    {
        ...
        var tokenUri = MareAuth.RenewOAuthTokenFullPath(new Uri(_serverManager.CurrentAuthServiceUri));
        HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, tokenUri.ToString());
