...
public partial class FileDownloadManager : DisposableMediatorSubscriberBase
{
    ...
    {
        var requestUri = MareFiles.ServerFilesProfileImageUpload(_orchestrator.FilesCdnUri!, imageUsage);
        ...
        return await _orchestrator.SendRequestAsync(HttpMethod.Post, requestUri, byteArrayContent, ct, withToken: true).ConfigureAwait(false);
    }
    ...
    public async Task DownloadProfileImageAsync(string uid, CancellationToken ct, Action<byte[]> setImageBytes)
    {
        ...
        {
            Logger.LogError(ex, "Failed to download profile image for {uid}", uid);
        }
    }
    ...
    private async Task<byte[]?> DownloadImageBytesAsync(string downloadUrl, CancellationToken ct)
    {
        ...
        using var response = await _orchestrator.SendRequestAsync(HttpMethod.Get, new Uri(downloadUrl), ct, 
    ...
    {
        var requestUri = MareFiles.ServerFilesProfileImageDownload(_orchestrator.FilesCdnUri!, uid);

        using var response = await _orchestrator.SendRequestAsync(HttpMethod.Get, requestUri, ct, withToken: true).ConfigureAwait(false);
