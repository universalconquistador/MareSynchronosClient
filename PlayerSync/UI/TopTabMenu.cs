...
public class TopTabMenu : IMediatorSubscriber
{
    ...
    private void DrawSyncshellMenu(float availableWidth, float spacingX)
    {
        ...
        using (ImRaii.Disabled(_pairManager.GroupPairs.Select(k => k.Key).Where(k => !k.PublicData.IsZoneSync).Distinct().Count() >= _apiController.ServerInfo.MaxGroupsJoinedByUser))
        {
